...
- **Verify:** HTML output shows two visually distinct sections ✅ (imports clean, tests pass; full visual verification in Task 4.2)
...
- Pass both groups to `generate_chart()` with `tickers_intl` and `tickers_kr` params
- Terminal output shows both groups with a separator

- **Verify:** full dashboard runs with all 6 tickers ✅ (imports clean, tests pass)
...
- **Verify:** allocation percentages for KR tickers sum to 100% ✅ (base_weights=1.0, min_weights=0.60)
...
- Each source (yfinance/pykrx/krx_gold) uses its own fetcher for historical data
...
- Replaced Plotly equity curves with clean HTML tables
- Per-ticker table grouped by period (5y, 10y) showing: Flat Return, Score Return, Edge, Max DD
- Portfolio table showing flat vs score allocation comparison
- Color-coded edge (green positive, red negative)
- No Plotly dependency for backtest output — static HTML, loads instantly
...
Set weights for the Korean group (mirrors international allocation):
- TIGER S&P500: base=0.55, min=0.40 (core holding)
- TIGER 나스닥100: base=0.15, min=0.05 (growth satellite)
- 금현물 (KRX): base=0.30, min=0.20 (hedge)

Sum: base=1.00, min=0.65 (leaves 35% for dynamic rebalancing).
...
- `docs/scoring.md` — score methodology, per-ticker MA weights/thresholds, suggestion tiers
- `docs/backtest.md` — backtest modes, strategies, metrics, data sources
- `docs/korean-data-sources.md` — KRX Gold API setup, pykrx usage, .env config, caching
- `docs/configuration.md` — all config.py options explained

**Trimmed `README.md` to:**
- Project one-liner + features
...
- **Verify:** README is under ~80 lines, all detailed info is findable in `docs/` ✅ (~70 lines)

### Task 6.3: Final integration test ✅

- Run `uv run python -m src.main` — dashboard with all 6 tickers ✅
- Run `uv run python -m src.main --backtest` — backtest for all tickers (verified in Task 5.1)
- Run `uv run pytest` — all 45 tests pass ✅
- Verify HTML outputs render correctly in browser ✅

### Task 6.4: Final verification of old file cleanup ✅

- Confirm Task 1.12's deletion of root-level files is complete ✅ (no stale files)
- Grep for any remaining imports referencing old paths ✅ (none found)
